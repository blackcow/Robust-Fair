...
def match_weight(label, diff0, diff1):
...
def cost_sensitive(lam0, lam1, beta):
...
def best_model(model, train_loader, optimizer, LayerOneTrainer, diff0, diff1, epoch, beta, device, rounds):
    ...
    for j in range(rounds):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            loss = torch.sum(weight0 * (loss_natural + beta * loss_robust1)) / torch.sum(weight0)
