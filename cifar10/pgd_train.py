...
def train(model, train_loader, optimizer, adversary, configs, epoch, device):
...
def main(args):
    ...
    for epoch in range(1, maxepoch + 1):
        ...
        train(model, train_loader, optimizer, adversary, configs, epoch, device)
        ...
        if (save_model and epoch % 10  == 0):
            if os.path.isdir('base/models/'):
                print('Save model.')
                torch.save(model.state_dict(), 'base/models/'+ 'pgd_' +str(epoch) + "_.pt")
            else:
