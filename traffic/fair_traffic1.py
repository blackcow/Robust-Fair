...
def main(args):
    ...
    if use_gpu:
        model.cuda()
    model.load_state_dict(torch.load('models/nadv_model_8.pth'))
    ...
    for now_epoch in range(1, maxepoch + 1):
        ...
        lmbda0 = lmbda[0:17] + rate / 10 * torch.clamp(gamma0, min = 0)
        lmbda0 = torch.clamp(lmbda0, min = 0)

        lmbda1 = lmbda[17:34]  + rate / 10 * torch.clamp(gamma1, min = 0)
        lmbda1 = torch.clamp(lmbda1, min = 0)
        print(lmbda1)

        lmbda2 = lmbda[34:51] +  5 * rate * (gamma1)
