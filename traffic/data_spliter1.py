    ...
    for i, sub_dir in enumerate(sub_dirs):
        ...
        if sub_dir_item_cnt[i] > 400:
            ...
            for item_idx in train_array:
                ...
                ## adjust contrast
                if '.ppm' in source_file:
                    img = cv2.imread(source_file)
                    img_grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    contrast = img_grey.std()

                    if contrast < 20:
                        alpha = 30 / contrast  # Contrast control (1.0-3.0)
                        adjusted = cv2.convertScaleAbs(img, alpha=alpha, beta=0)
                        cv2.imwrite(dst_file, adjusted)
                    else:
            ...
            for item_idx in test_array:
                ...
                if '.ppm' in source_file:
                    ...
                    if contrast < 20:
                        alpha = 30 / contrast  # Contrast control (1.0-3.0)
                        adjusted = cv2.convertScaleAbs(img, alpha=alpha, beta=2)
                        cv2.imwrite(dst_file, adjusted)
                    else:
                        shutil.copyfile(source_file, dst_file)
                else:
...
split_dataset_into_3('data/Images2',40,50)